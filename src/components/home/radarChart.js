  ...
  useEffect(() => {
    ...
    DrawRadarChart(
      ...
      [],
      {
        ...
        axis: ["dog", "cat", "alpaca", "panda", "penguin", "elephant", "Jian"],
      },
...
function DrawRadarChart(id, data, options, allSettingsParamValue) {
  var cfg = {
  ...
  const resetDataNew = () => {
    ...
    allAxis.forEach((ax, ind) => {
      console.log(ax, ind, allSettingsParamValue.targetProbability[ax]);
      dataNew.data.push({
        axis: ax,
        value: 0.01,
        ind: ind,
    ...
    console.log('init', dataNew);
    // drawblob([dataNew.data.filter((d) => d.value > 0)]);
  };
  resetDataNew();
  ...
  axis
    ...
    // REF: https://css-tricks.com/almanac/properties/s/stroke-linecap/
    ...
    .on("click", function (d, i) {
      ...
      x2 = Math.abs(x2);
      y2 = Math.abs(y2);
      var closestPoint = findNearestPointOnLine(clickX, clickY, x1, y1, x2, y2);
  ...
  function drawblob(receivedData) {
    ...
    var blobWrapper = g.selectAll(".radarWrapper").data(receivedData);
    ...
    blobRadarCircle
      ...
      .attr("cx", function (d, i) {
        ...
        return rScale(d.value) * Math.cos(angleSlice * d.ind - Math.PI / 2);
        // return rScale(d.value) * Math.cos(angleSlice * i - Math.PI / 2)
      })
      .attr("cy", function (d, i) {
        return rScale(d.value) * Math.sin(angleSlice * d.ind - Math.PI / 2);
